...
try:
    ...
    import os
    import sys
    vm_ex, vd_ex, cy_ex, a_ex, rox_ex, c_end = Fore.LIGHTRED_EX, Fore.LIGHTGREEN_EX, Fore.LIGHTCYAN_EX, Fore.LIGHTYELLOW_EX, Fore.LIGHTMAGENTA_EX, Style.RESET_ALL
...
def scan(target):
        ...
        try: 
            p_start = int(input(f'{vd_ex}[P¹] Insert starting port range number:\n>>>{c_end} '))
            p_start.shuffle()
            p_end = int(input(f'{vd_ex}[P²] Insert final port range number:\n>>>{c_end} '))
            p_end.shuffle()
            print(f'{rox_ex}[=] Scanning Ports {p_start} to {p_end} at ──› {c_end}' + str(target))
        except KeyboardInterrupt:
            print(f'\n\t{a_ex}Keyboard Interrupt... Bye!{c_end}\n')
...
try:
    set_timeout = float(input(f'{cy_ex}[+] Insert timeout number{c_end} {a_ex}(Float nº recommended: >0.7){c_end}{cy_ex}:\n>>> {c_end}'))
...
def scan_port(ipaddress, port):
    try:
        ...
        try:
            ...
            print(f'{vd_ex}[+] Port {c_end}' + str(port) + f'/tcp {vd_ex}is open. Service ──› {c_end}' + str(banner.decode().strip('\n'))) #Banner
        except:
            print(f'{vd_ex}[+] Port {c_end}' + str(port) + f'/tcp {vd_ex}is open. Service ──› {c_end}<unable to find>')
    except:
        pass
